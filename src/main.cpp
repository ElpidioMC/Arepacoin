...
unsigned int nTargetSpacing = 1 * 60; // 1 minutes
unsigned int nTargetSpacing_v2 = 1 * 60; // 1 minutes
...
static const int64_t nTargetTimespan_v2 = 16 * 60;  // 60 mins
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "arepacoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "aasdfaf afs as df";
        CTransaction txNew;
        txNew.nTime = 1474758308;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1474758308;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 3261211 : 3261211;
        ...
        assert(block.hashMerkleRoot == uint256("0xd4082f34d13c3e5c4ad7057cd6548bc44274a6234e168157bbb760545aec22e1"));
...
unsigned char pchMessageStart[4] = { 0xb6, 0x60, 0x02, 0xf5 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // dvrs: send latest proof-of-work block to allow the
